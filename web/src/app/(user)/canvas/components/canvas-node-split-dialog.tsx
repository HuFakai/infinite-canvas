...
export function CanvasNodeSplitDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (params: CanvasImageSplitParams) => void }) {
    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={940} centered destroyOnHidden>
            <div className="space-y-5">
                ...
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]">
                    <div className="space-y-3">
                        {image ? (
                            <CanvasImageEditorViewport image={image} stageRef={previewRef} className="h-[430px]">
                                <img src={dataUrl} alt="" className="absolute inset-0 size-full object-fill opacity-95" draggable={false} />
                            ...
                            <div className="grid h-[430px] place-items-center rounded-lg border text-sm opacity-60">正在读取图片</div>
                        )}
...
function SplitGrid({
    ...
    active: ActiveLine;
    onPointerDown: (axis: "horizontal" | "vertical", index: number, event: ReactPointerEvent) => void;
}) {
    return (
